...
## Active Task
env provisioning + end-to-end smoke test against live tenants.
...
- 2026-04-19 — Tenants live: Supabase (migrations + seed + photos bucket), Auth0 (SPA + API `https://terra-triage.local/agent` + `referral:send` scope + M2M app), Resend (`onboarding@resend.dev` sender). PAR gated behind `AUTH0_PAR=1` env (tenant toggle wasn't exposed in UI); default off — M2M fallback still demos scoped agent token primitive. `/api/auth/login` verified 307 → Auth0 `/authorize`. Intake at `/report` (not `/triage`).
